...
class QdrantIndex(BaseIndex):
    ...
    def add(
        ...
        for route, utterance, embedding in zip(routes, utterances, embeddings):
            # Generate deterministic UUIDs based on the route and utterance
            # Points are overwritten if the same UUID is used
            id = uuid.uuid5(UUID_NAMESPACE, f"{route}#{utterance}").hex
            ids.append(id)

            payloads.append({"sr_route": route, "sr_utterance": utterance, "id": id})
